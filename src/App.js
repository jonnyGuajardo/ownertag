...
function Inp({ label,value,onChange,placeholder,type="text",err,as }) {
  ...
  return (
    <div style={{ marginBottom:14 }}>
      ...
      {as==="select"
        ? <select value={value} onChange={e=>onChange(e.target.value)} style={base}>{placeholder}</select>
        : as==="textarea"
...
function SignaturePad({ onSave, onCancel, title }) {
  ...
  function move(e) {
    ...
    ctx.lineTo(pos.x,pos.y); ctx.strokeStyle="#1e1e2e"; ctx.lineWidth=2.5;
...
function ContractView({ contract, session, onSigned, onCancel }) {
  const [step, setStep]         = useState("view"); // view | sign_sender | sign_receiver | done
  ...
  async function saveSig(dataUrl) {
    ...
    const update    = { [field]:dataUrl, [timeField]:new Date().toISOString() };
    const { data:updated } = await supabase.from("transfer_contracts").update(update).eq("id",contract.id).select().single();
    ...
    <div>
      ...
      <div style={{ border:"2px solid #e5e7eb",borderRadius:12,padding:20,marginBottom:20,background:"#fafafa" }}>
        <div style={{ textAlign:"center",marginBottom:20 }}>
          ...
          <div style={{ fontSize:12,color:"#9ca3af",marginTop:4 }}>Documento generado el {new Date().toLocaleDateString("es-MX")}</div>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
            <div style={{ background:"#fff",borderRadius:8,padding:10,border:"1px solid #e5e7eb" }}>
              <div style={{ color:"#9ca3af",fontSize:11 }}>Vendedor (Emisor)</div>
              <div style={{ fontWeight:600,fontSize:12,wordBreak:"break-all" }}>{contract.sender_email}</div>
            </div>
            <div style={{ background:"#fff",borderRadius:8,padding:10,border:"1px solid #e5e7eb" }}>
              <div style={{ color:"#9ca3af",fontSize:11 }}>Comprador (Receptor)</div>
        ...
        <div style={{ background:"#f0f0ff",borderRadius:10,padding:14,marginBottom:16,fontSize:12,color:"#4338ca",lineHeight:1.6 }}>
          El vendedor declara ser el legítimo propietario del bien descrito y cede todos los derechos de propiedad al comprador. El comprador acepta las condiciones del bien en el estado actual. La transferencia se considera efectiva únicamente cuando ambas partes hayan firmado digitalmente este documento en la plataforma OwnerTag.
        </div>

        {/* Signatures */}
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
          {[
            { label:"Firma del Vendedor", sig:contract.sender_signature, date:contract.sender_signed_at },
            { label:"Firma del Comprador", sig:contract.receiver_signature, date:contract.receiver_signed_at },
          ].map(({ label,sig,date })=>(
            <div key={label} style={{ border:"1px solid #e5e7eb",borderRadius:10,padding:12,background:"#fff",minHeight:100 }}>
              ...
              {sig
                ? <>
                    <img src={sig} alt="firma" style={{ width:"100%",height:60,objectFit:"contain",border:"1px dashed #c7d2fe",borderRadius:6 }} />
                    <div style={{ color:"#16a34a",fontSize:11,marginTop:4 }}>✅ Firmado {date?.slice(0,10)}</div>
                  </>
      ...
      {needsMySig && (
        <div style={{ marginBottom:16 }}>
          ...
          <Btn onClick={()=>setStep(isSender?"sign_sender":"sign_receiver")} full loading={loading}>
            ✍️ Firmar como {isSender?"Vendedor":"Comprador"}
          </Btn>
        </div>
      )}
      ...
      {!needsMySig && contract.status!=="completed" && (
        <div style={{ background:"#f0f0ff",border:"1px solid #c7d2fe",borderRadius:10,padding:12,marginBottom:16 }}>
          <div style={{ fontSize:13,color:"#4338ca" }}>⏳ Ya firmaste. Esperando la firma de la otra parte para completar la transferencia.</div>
        </div>
      )}
...
export default function App() {
  ...
  async function doTransfer() {
    ...
    if(xferContract) {
      // Create contract, don't transfer yet
      await supabase.from("transfer_contracts").insert({
        product_id:selected.id,
        sender_email:session.email,
        receiver_email:xferEmail,
        product_name:selProduct.name,
        product_serial:selProduct.serial,
        market_value:selProduct.market_value||"",
        status:"pending",
      });
      // Notify receiver via notification
      await supabase.from("notifications").insert({ user_id:session.id,type:"contract",title:"📄 Contrato enviado",message:`Enviaste un contrato de compra-venta para "${selProduct.name}" a ${xferEmail}. Esperando su firma.`,product_id:selected.id });
      await fetchNotifications(session);
      await fetchPendingContracts(session);
          ...
          <div>
            ...
            {pendingContracts.length>0&&(
              <div style={{ marginBottom:20 }}>
                <div style={{ fontWeight:700,fontSize:15,marginBottom:10,color:"#111" }}>📄 Contratos pendientes de firma</div>
                {pendingContracts.map(c=>(
                  <div key={c.id} style={{ background:"#fef9c3",border:"1px solid #fcd34d",borderRadius:14,padding:16,marginBottom:10 }}>
                    ...
                    <div style={{ fontSize:13,color:"#92400e",marginBottom:4 }}>
                      {c.sender_email===session.email ? `Comprador: ${c.receiver_email}` : `Vendedor: ${c.sender_email}`}
                    </div>
                    <div style={{ fontSize:12,color:"#9ca3af",marginBottom:10 }}>
                      {c.sender_signature&&c.sender_email===session.email ? "✅ Ya firmaste" : !c.sender_signature&&c.sender_email===session.email ? "⏳ Tu firma pendiente" : ""}
                      {c.receiver_signature&&c.receiver_email===session.email ? "✅ Ya firmaste" : !c.receiver_signature&&c.receiver_email===session.email ? "⏳ Tu firma pendiente" : ""}
                    </div>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
              ...
              <div style={{ marginBottom:14 }}>
                ...
                <select value={mServiceType} onChange={e=>setMServiceType(e.target.value)}
                  style={{ width:"100%",border:"1px solid #e5e7eb",borderRadius:8,padding:"10px 12px",fontSize:14,outline:"none",boxSizing:"border-box",color:"#111",background:"#fafafa" }}>
        ...
        <Modal title="Transferir propiedad" onClose={()=>{ setShowXfer(false);setXferEmail("");setXferDocs({});setXferContract(false); }}>
          ...
          <label style={{ display:"flex",alignItems:"flex-start",gap:12,background:xferContract?"#f0f0ff":"#f8fafc",border:"1px solid "+(xferContract?"#c7d2fe":"#e5e7eb"),borderRadius:10,padding:14,marginBottom:16,cursor:"pointer" }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12,color:"#6b7280" }}>Ambas partes deberán firmar digitalmente antes de que se complete la transferencia.</div>
            </div>
          </label>
      ...
      {showContract&&(
        <Modal title="📄 Contrato de Compra-Venta" onClose={()=>setShowContract(null)} wide fullscreen>
          <ContractView
            ...
            onSigned={(completed)=>{
              if(completed){
                toast("🎉 ¡Contrato completado! Transferencia realizada.");
                fetchProducts(session);
                fetchPendingContracts(session);
              } else {
                toast("✅ Firma guardada. Esperando la otra parte.");
                fetchPendingContracts(session);
              }
        ...
        <div style={{ position:"fixed",inset:0,background:"#000d",display:"flex",flexDirection:"column",zIndex:200 }}>
          ...
          <div style={{ flex:1,overflow:"auto",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
            {["jpg","jpeg","png","gif","webp"].includes(previewFile.ext)&&(
              <img src={previewFile.url} alt={previewFile.name} style={{ maxWidth:"100%",maxHeight:"100%",borderRadius:8,boxShadow:"0 8px 40px #0008" }} />
            )}
            {previewFile.ext==="pdf"&&(
              <iframe src={previewFile.url} title={previewFile.name} style={{ width:"100%",height:"100%",minHeight:"70vh",border:"none",borderRadius:8 }} />
            )}
            {["doc","docx","xls","xlsx"].includes(previewFile.ext)&&(
              <div style={{ background:"#fff",borderRadius:16,padding:40,textAlign:"center",maxWidth:400 }}>
                ...
                <div style={{ color:"#6b7280",fontSize:14,marginBottom:24 }}>Este tipo de archivo no se puede previsualizar en el navegador.</div>
